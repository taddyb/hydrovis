...
CREATE INDEX IF NOT EXISTS idx_inundation_prvi_geo_geom ON fim_ingest.ana_inundation_prvi_geo USING GIST (geom);
ANALYZE external.building_footprints_fema;
ANALYZE fim_ingest.ana_inundation_prvi_geo;
SELECT
	buildings.build_id,
    buildings.occ_cls,
    buildings.prim_occ,
...
JOIN fim_ingest.ana_inundation_prvi_geo fim_geo ON ST_INTERSECTS(fim_geo.geom, buildings.geom)
JOIN fim_ingest.ana_inundation_prvi_flows flows ON fim_geo.hand_id = flows.hand_id
JOIN fim_ingest.ana_inundation_prvi fim ON fim_geo.hand_id = fim.hand_id;
