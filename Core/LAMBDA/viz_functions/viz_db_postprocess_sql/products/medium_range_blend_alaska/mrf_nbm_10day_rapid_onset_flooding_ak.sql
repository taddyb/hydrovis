DROP TABLE IF EXISTS publish.mrf_nbm_10day_rapid_onset_flooding_alaska;
	...
	WITH floodstart AS (
		...
		WITH pct_change_by_hour AS
			...
			WITH forecasts_full AS
				...
				FROM series
				LEFT OUTER JOIN ingest.nwm_channel_rt_mrf_nbm_ak AS forecasts ON series.feature_id = forecasts.feature_id AND series.forecast_hour = forecasts.forecast_hour -- Left outer join to the forecasts table (so that all hours are always present)
