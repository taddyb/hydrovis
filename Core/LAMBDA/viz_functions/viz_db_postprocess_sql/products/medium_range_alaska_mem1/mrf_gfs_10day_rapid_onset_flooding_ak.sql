DROP TABLE IF EXISTS publish.mrf_gfs_10day_rapid_onset_flooding_alaska;
...
WITH rapid_onset AS (
	...
	WITH floodstart AS (
		...
		WITH pct_change_by_hour AS
			...
			WITH forecasts_full AS
				...
				FROM series
				LEFT OUTER JOIN ingest.nwm_channel_rt_mrf_gfs_ak_mem1 AS forecasts ON series.feature_id = forecasts.feature_id AND series.forecast_hour = forecasts.forecast_hour -- Left outer join to the forecasts table (so that all hours are always present)
	...
	WHERE -- This is where the main forecast filter conditions go
		(thresholds.high_water_threshold > 0) AND -- Don't show reaches with invalid high_water_threshold values
